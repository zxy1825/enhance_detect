...
class FeatureMapFile:
    ...
    # 将直方图分布转换成概率分布
    def convert_hist_to_prob(self, hist_list):
        prob_list = []
    ...
    def calculate_fft(self):
        fourier_transformed = np.fft.fft2(self.image, axes=(1, 2))
        fourier_transformed_shifted = np.fft.fftshift(fourier_transformed, axes=(1, 2))
    ...
    def save_prob_to_file(self):
        # 准备数据并创建DataFrame
        data = []
        bins_list, hist_list = self.calculate_hist()
        prob_list = self.convert_hist_to_prob(hist_list)
        for bins, prob in zip(bins_list, prob_list):
